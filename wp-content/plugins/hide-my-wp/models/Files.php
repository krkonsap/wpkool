...
class HMW_Models_Files {
    ...
    public function showFile($url) {
        ...
        if (empty($this->_replace['from']) && empty($this->_replace['to'])) {
            if (!isset(HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->_replace['from']) && !isset(HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->_replace['to'])) {
                HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->buildRedirect();
            }

            //Verify only the rewrites
            $rewrite = HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->_replace['rewrite'];
            $rewrite_from = HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->_replace['from'];
            $rewrite_to = HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->_replace['to'];
            foreach ($rewrite as $index => $value) {
                //add only the paths
                if ($value && isset($rewrite_to[$index]) && substr($rewrite_to[$index], -1) == '/') {
        ...
        if (strpos($new_url, '/wp-content/') !== false && strpos($new_url, '/busting/') === false) {
            ...
            if ($ext = $this->isFile($new_url)) {
                ...
                if (file_exists($new_path)) {
                    ...
                    if (strpos($new_url, '.js')) {
                        //$content = HMW_Classes_ObjController::getClass('HMW_Models_Rewrite')->find_replace($content);

...
class HMW_Models_Rewrite {
    ...
    public function getBuffer($buffer) {
        ...
        if ($hmw_process_buffer) {
            ...
            if (HMW_Classes_Tools::isAjax() && (HMW_FORCEPATH || function_exists('is_user_logged_in') && !is_user_logged_in())) {
                //replace the URLs in Ajax
                $buffer = $this->find_replace($buffer);
            } elseif (HMW_Classes_Tools::getOption('hmw_mode') <> 'default') {//If not in default mode
                ...
                if (HMW_Classes_Tools::isHtmlHeader()) {
    ...
    public function hidePluginNames() {
        ...
        foreach ($all_plugins as $plugin) {
            if (is_plugin_active($plugin)) {
                ...
                $dbplugins['from'][] = urlencode(rtrim(plugin_dir_path($plugin), '/')) . '/';;
            }
        }
    ...
    public function flushRewrites() {
        ...
        if (!HMW_Classes_Tools::isWpengine()) {
            $form .= '<form method="POST" style="margin: 8px 0;">
                        ...
                        <input type="submit" class="btn rounded-0 btn-warning save" value="' . __("Can't configure it now. Run without rewrites", _HMW_PLUGIN_NAME_) . '" />
                    </form>';
        }
        $path = parse_url(site_url(), PHP_URL_PATH);
        if ($path) $home_root = trailingslashit($path);
        ...
        if (HMW_Classes_Tools::isIIS()) {
            ...
            if (!empty($this->_rewrites)) {
                foreach ($this->_rewrites as $rewrite) {
                    if (strpos($rewrite['to'], 'index.php') === false && (strpos($rewrite['to'], 'wp-content') !== false || strpos($rewrite['to'], 'wp-includes') !== false))
    ...
    public function prepareFindReplace($content = '') {
        ...
        if (isset($this->_replace['from']) && isset($this->_replace['to']) && !empty($this->_replace['from']) && !empty($this->_replace['to'])) {
            ...
            $find = array_map(array($this, 'removeSchema'), (array)$this->_replace['from']);
            $replace = array_map(array($this, 'removeSchema'), (array)$this->_replace['to']);

            //change the javascript urls
            $findencoded = array_map(array($this, 'removeSchemaEncoded'), (array)$this->_replace['from']);
            $replaceencoded = array_map(array($this, 'removeSchemaEncoded'), (array)$this->_replace['to']);
            //change the javascript urls
            $findencodedfinal = array_map(array($this, 'removeSchemaEncodedFinal'), (array)$this->_replace['from']);
            $replaceencodedfinal = array_map(array($this, 'removeSchemaEncodedFinal'), (array)$this->_replace['to']);
        }
    ...
    public function replaceLinks($found) {
        ...
        if (strpos($found[0], '//') === false && strpos($found[0], '\/\/') === false) {
            return $found[1] . str_ireplace($this->_replace['from'], $this->_replace['to'], $this->rel2abs($url));
        } else {
            return $found[0];
        }

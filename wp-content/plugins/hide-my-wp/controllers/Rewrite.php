...
class HMW_Controllers_Rewrite extends HMW_Classes_FrontController {
    ...
    public function __construct() {
        ...
        if (!WP_CACHE && !HMW_Classes_Tools::getOption('hmw_laterload')) {
    ...
    public function initHooks() {
        ...
        if (HMW_Classes_Tools::isPermalinkStructure()) {
            ...
            add_action('wp_logout', array($this->model, 'wp_logout'), 99);

            //change the admin url
            add_filter('lostpassword_url', array($this->model, 'lostpassword_url'), 1, 1);
            add_filter('register', array($this->model, 'register_url'), 1, 1);
            add_filter('login_url', array($this->model, 'login_url'), 1, 1);
            add_filter('logout_url', array($this->model, 'logout_url'), 1, 2);
            ...
            add_filter('network_site_url', array($this->model, 'site_url'), 1, 3);

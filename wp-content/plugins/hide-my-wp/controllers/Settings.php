...
class HMW_Controllers_Settings extends HMW_Classes_FrontController {
    ...
    public function init() {
        ...
        if (HMW_Classes_Tools::getOption('hmw_mode') == 'lite') {
            if(gmdate('Y-m-d') <= '2018-11-17') {
                HMW_Classes_Error::setError(sprintf(__('%sSave Big until 17 Nov 2018%s: Get %s77%% OFF%s on Hide My WP Ghost Unlimited Websites License. %sSee offer%s', _HMW_PLUGIN_NAME_), '<strong style="color: red">', '</strong>', '<strong style="color: red">', '</strong>', '<a href="https://wpplugins.tips/hide-my-wp-special-discount/" target="_blank" style="font-weight: bold">', '</a>'));
            }elseif(gmdate('Y-m-d') >= '2018-11-24' && gmdate('Y-m-d') <= '2018-11-25') {
                HMW_Classes_Error::setError(sprintf(__('%sBlack Friday Sale%s: Get %s77%% OFF%s on Hide My WP Ghost Unlimited Websites License until 24 Nov 2018. %sSee the offer%s', _HMW_PLUGIN_NAME_), '<strong style="color: red">', '</strong>', '<strong style="color: red">', '</strong>', '<a href="https://wpplugins.tips/black-friday-2018-hide-my-wp/" target="_blank" style="font-weight: bold">', '</a>'));
            }else {
                HMW_Classes_Error::setError(sprintf(__('%sUnlock Hide My WP Ghost%s premium features with %sonly $23.99%s. Use the coupon %s1HIDEMYWP20%s. Limited stock. %sUnlock Now%s', _HMW_PLUGIN_NAME_), '<strong style="color: red">', '</strong>', '<strong style="color: red">', '</strong>', '<strong  style="color: red">', '</strong>', '<a href="https://hidemywp.co/wordpress_update" target="_blank" style="font-weight: bold">', '</a>'));
            }
        }
    ...
    public function getAdminTabs($current = null) {
        ...
        $content .= '<div  class="m-0 p-4 font-dark text-logo"><a href="http://hidemywp.co/" target="_blank"><img src="' . _HMW_THEME_URL_ . 'img/logo.png" class="ml-0 mr-2" style="width:30px;"></a>' . __('Hide My WP', _HMW_PLUGIN_NAME_) . ' <span style="color: #d6cdd1">' . _HMW_VER_NAME_ . '</span></div>';
        foreach ($this->tabs as $location => $tab) {
            ...
            $content .= '<a class="m-0 p-4 font-dark hmw_nav_item ' . $class . ' fa fa-' . $tab['icon'] . '" href="' . HMW_Classes_Tools::getSettingsUrl() . ($location <> 'hmw_permalinks' ? '&tab=' . $location : '') . '">
                <span>' . $tab['title'] . '</span>
                <span class="hmw_nav_item_description">' . $tab['description'] . '</span>
            </a>';
        }

...
class HMW_Classes_Tools extends HMW_Classes_FrontController {
    ...
    public static function getOptions($safe = false) {
        ...
        self::$init = array(
            ...
            'hmw_text_mapping' => json_encode(
                array(
                    'from' => array('wp-image', 'wp-caption', 'wp-custom'),
                    'to' => array('image', 'caption', 'custom'),
                )
            ),
        );
        self::$default = array(
    ...
    public static function checkApi($email = null, $redirect_to = '') {
        ...
        if (isset($email) && $email <> '') {
            $args = array('email' => $email, 'url' => home_url(), 'source' => _HMW_PLUGIN_NAME_);
            $response = self::hmw_remote_get(_HMW_API_SITE_ . '/api/free/token', $args, array('timeout' => 10));
        } elseif (self::getOption('hmw_token')) {
            $args = array('token' => self::getOption('hmw_token'), 'url' => home_url(), 'source' => _HMW_PLUGIN_NAME_);
            $response = self::hmw_remote_get(_HMW_API_SITE_ . '/api/free/token', $args, array('timeout' => 10));
        } else {
            return $check;
        }
    ...
    public static function sendEmail() {
        ...
        $message .= $line;
        $message .= "SPECIAL OFFER: 20% Discount! Use coupon 1HIDEMYWP20 for the Hide My WP Ghost" . "\n";
